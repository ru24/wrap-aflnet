...
struct queue_entry {
  ...
  *next_100;       /* 100 elements ahead               */
  ...
  u8 *inputfaults;           /* fault injectionの記録用          */
...
// ハッシュ関数と比較関数
#define FNV1_HASH(key) fnv1a_64(&(key), sizeof(key))
#define FNV1_EQUAL(a, b) ((a) == (b))

// uint64_t をキーとして使用するハッシュテーブルの定義
// KHASH_INIT(fnv1, uint64_t, int, 1, FNV1_HASH, FNV1_EQUAL)

// 文字列をキーとして使用する別のハッシュテーブル


// KLIST_INIT(fnv1, uint64_t, free)

/* Hash table/map and list */
klist_t(lms) *kl_messages;      // lms
khash_t(hs32) *khs_ipsm_paths;  // hs32
khash_t(hms) *khms_states;      // hms
...
u32 get_unique_state_count(unsigned long *state_sequence, unsigned int state_count) {
...
u8 is_state_sequence_interesting(unsigned long *state_sequence, unsigned int state_count) {
...
/* Listをファイルに保存 */
void save_SFIList_response_code_to_file(InputFaults *inputfaults, unsigned char *fname)
{
  int fd;
  FILE* fp;
  ...
  int i;
  for (i = 0; i < inputfaults->current_size; i++) {
    fprintf(fp, "%d\n", inputfaults->faults[i].response_code);
  }
...
void update_state_aware_inputfaults (u8 *fn) 
{
  ...
  // fid用のディレクトリに保存
  char *fid_path = alloc_printf("%s/SFI_List/fid/%s", out_dir, base_name);
...
bool is_target_state_id_included(InputFaults* inputfaults, int target_state_id) {
  if (inputfaults == NULL || inputfaults->faults[0].fid == '\0') {
...
void create_input_faults_mutated_from_qfname(const char *q_fname) {
  ...
  int *is_fi_values = read_values_from_file(is_fi_file, &is_fi_count);
  int *fid_values = read_values_from_file(fid_file, &fid_count);
  int *response_code_values = read_values_from_file(response_code_file, &response_code_count);
...
// 関数定義: 条件を満たすfidをstate_sequence配列として返す
unsigned long* find_matching_fids(unsigned long *state_sequence, int state_count, InputFaults *Input_faults) {
    uint64_t *result_array = (uint64_t *)malloc(sizeof(uint64_t) * state_count);
    ...
    for (int i = 0; i < state_count; i++) {
        unsigned long state_value = state_sequence[i];
    uint64_t bitfield = 0; // 各 state_value に対応するビットフィールド
        
        for (int j = 0; j < Input_faults->current_size; j++) {
            if (state_value == Input_faults->faults[j].response_code) {
                unsigned long fid = Input_faults->faults[j].fid;

                // fid をビットシフトして記録
                if (fid < 32) { // fid が 32 未満の場合のみ記録
                    bitfield |= (1UL << fid);
                }
            }
        }
            // 結果を格納
        result_array[i] = (uint64_t)state_value << 32 | bitfield;
    }
  return result_array;
}

// 関数定義: unsigned long 配列を 32 ビット右シフトし、unsigned int 配列に格納
void shift_state_sequence(unsigned long state_sequence[], int state_count, unsigned int shifted_array[]) {
    for (int i = 0; i < state_count; i++) {
        shifted_array[i] = (unsigned int)(state_sequence[i] >> 32); // 32 ビット右シフト
    }
}
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  // unsigned long *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);

  // extract_response_codes の戻り値をそのまま受け取る
unsigned int *temp_state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);

// unsigned long 配列を作成して変換
unsigned long *state_sequence = malloc(state_count * sizeof(unsigned long));
if (!state_sequence) {
...
for (unsigned int i = 0; i < state_count; i++) {
    state_sequence[i] = (unsigned long)temp_state_sequence[i];
}

  state_sequence = find_matching_fids(state_sequence, state_count, input_faults);
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
    ...
    if (state_count > 1) {
      unsigned long prevStateID = state_sequence[0];

      for(i=1; i < state_count; i++) {
        unsigned long curStateID = state_sequence[i];
        char fromState[STATE_STR_LEN], toState[STATE_STR_LEN];
        snprintf(fromState, STATE_STR_LEN, "%ld", prevStateID);
        snprintf(toState, STATE_STR_LEN, "%ld", curStateID);
        ...
        if (!from) {
          ...
          state_ids[state_ids_count++] = (unsigned int)(prevStateID >> 32); // 32 ビット右シフト
        ...
        if (!to) {
          ...
          khms_states; // StateID(int) をKeyとして、 <FID, state_info_t>をValueとする辞書 dict
          intermediate_states; // FID をKeyとして state_info_t> dict
          ...
          //Insert this into the state_ids array too
          state_ids = (u32 *) ck_realloc(state_ids, (state_ids_count + 1) * sizeof(u32));
          state_ids[state_ids_count++] = (unsigned int)(curStateID >> 32); //右シフト
  ...
  for(i = 0; i < q->region_count; i++) {
    ...
    if (regional_state_count > 0) {
      ...
      unsigned long reachable_state_id = q->regions[i].state_sequence[regional_state_count - 1];
      ...
      if (k != kh_end(khms_states)) {
        ...
        //Insert this into the state_ids array too
        state_ids = (u32 *) ck_realloc(state_ids, (state_ids_count + 1) * sizeof(u32));
        state_ids[state_ids_count++] = (unsigned int)(reachable_state_id >> 32);
  ...
  for(i = 0; i < state_count; i++) {
    unsigned int state_id = (unsigned int)(state_sequence[i] >> 32);
...
static void maybe_delete_out_dir(void) {
  ...
  /* SFI_Listおよびそのサブディレクトリを削除 */
  fn = alloc_printf("%s/SFI_List/response_code", out_dir);
  if (delete_files(fn, "")) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/SFI_List/fid", out_dir);
  if (delete_files(fn, "")) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/SFI_List/is_fi", out_dir);
  if (delete_files(fn, "")) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/SFI_List", out_dir);
  if (delete_files(fn, "")) goto dir_cleanup_failed;
  ck_free(fn);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  tmp = alloc_printf("%s/SFI_List", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* SFI_Listのサブディレクトリ作成 */

  tmp = alloc_printf("%s/SFI_List/is_fi", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/SFI_List/fid", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/SFI_List/response_code", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
